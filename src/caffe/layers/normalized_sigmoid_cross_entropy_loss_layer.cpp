...
void NormalizedSigmoidCrossEntropyLossLayer<Dtype>::Forward_cpu(
  ...
  const Dtype* indicator = bottom[2]->cpu_data();
  ...
  for (int i = 0; i < count; ++i) {
    int n = (int) floor( i / outer_factor );
    int m = (int) floor( ( i % outer_factor ) / inner_factor );
    // loss -= input_data[i] * (target[i] - (input_data[i] >= 0)) -
    //     log(1 + exp(input_data[i] - 2 * input_data[i] * (input_data[i] >= 0)));
    loss -= indicator[n*dim_class + m] * ( input_data[i] * (target[i] - (input_data[i] >= 0)) -
        log(1 + exp(input_data[i] - 2 * input_data[i] * (input_data[i] >= 0))) );
  }
...
void NormalizedSigmoidCrossEntropyLossLayer<Dtype>::Backward_cpu(
  ...
  if (propagate_down[0]) {
    ...
    for ( int i = 0; i < count; ++i ) {
      int n = (int) floor( i / outer_factor );
      int m = (int) floor( ( i % outer_factor ) / inner_factor );
      bottom_diff[i] *= indicator[n*dim_class + m];
    }
    // Scale down gradient
